...
Verifies OTP and sets password/username.
**Body:** `{ email, otp, username, password }`
**Response:** `200 OK { user: { id, email, username }, accessToken }`
...
#### `GET /api/auth/me`
Gets profile from Bearer token.
**Header:** `Authorization: Bearer <token>`
**Response:** `200 OK { id, username, email }`

**Note on user IDs**: Authenticated requests passing a Bearer token use the Supabase `user.id` (UUID) as the userId in trips and members lists.

---

### 9.8 Notifications API

All notification endpoints require a valid `Authorization: Bearer <token>` header. The userId is inferred server-side from the token — it is **never** passed in the URL.

#### `GET /api/notifications`
Returns all notifications for the authenticated user.
**Query params:** `?tripId=<tripId>` (optional — filter to one trip)
**Response:**
```json
{
  "notifications": [
    {
      ...
      "tripId": "trip_abc123",
      "type": "trip_created",
      "message": "Your Tokyo adventure is ready!",
      "createdAt": "2026-09-07T14:29:16.711Z",
      "read": false
    }
  ]
}
...
**Trigger events and their recipients:**

| Event | Trigger | Recipients |
|---|---|---|
| `trip_created` | Trip created | Creator |
| `user_joined` | Member joins group trip | Admin + all existing members |
| `you_joined` | Member joins group trip | The joiner |
| `user_left` | Member leaves trip | Remaining members |
| `you_left` | Member leaves trip | The leaver |
| `user_removed` | Admin removes member | Remaining members |
| `you_removed` | Admin removes member | The removed member |
| `trip_deleted` | Admin deletes trip | All members |
| `itinerary_replanned` | Replan completes | All members |
| `milestone_reached` | Location triggers a checkpoint | All members |

Notification messages are dynamically generated by Groq (`openai/gpt-oss-20b` model) with an adventurous tone for positive events and a neutral tone for negative ones. A fallback message is used if Groq fails or times out (1500ms timeout).
